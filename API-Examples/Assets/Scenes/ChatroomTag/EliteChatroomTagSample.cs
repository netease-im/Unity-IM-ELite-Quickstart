 ...
 * 用户D没有设置自己的Tag。
 * 
 * 1，用户A发送了一条消息，该消息带有tagA,则本条消息能被用户CD收到
 * 2，用户A发送了一条消息，该消息没带tag, 系统会自动给本消息加上发送者的tag，故本条消息能被用户CD收到
...
public class EliteChatroomTagSample : MonoBehaviour
{
    ...
    //输入控件，用以获取登录聊天室时想要设置的tag
    InputField txtSelfTag;
    //输入控件，用以获取消息的tag
    InputField txtMessageTag;
    //输入控件，用以获取想要发送的消息
    ...
    {
        _ = Loom.Current;
        txtSelfTag = GameObject.Find("SelfTagInputField").GetComponent<InputField>();
        txtMessage = GameObject.Find("MessageInputField").GetComponent<InputField>();
        txtMessageTag = GameObject.Find("MessageTagInputField").GetComponent<InputField>();
    ...
    public void OnSendMessageClick()
    {
        ...
        //本消息要发给哪个tag的成员
        if(!string.IsNullOrEmpty(txtMessageTag.text))
        {
            JObject notifyTagsObj = new JObject();
            notifyTagsObj["tag"] = txtMessageTag.text;
            message.notifyTags = notifyTagsObj.ToString();
            Debug.Log("SendMessage notifyTags: " + message.notifyTags);
        }
